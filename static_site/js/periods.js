// Страница «Периоды» — каскад Вимшоттари-даши. Профиль из localStorage,
// расчёт клиентский (window.Jyotish.periods).
(function () {
  ...
  if (!profile || !profile.date || !profile.place) { need.style.display = 'block'; return; }
  try {
    var data = J.periods(profile);
    if (data.error) { need.style.display = 'block'; return; }
    render(data);
  } catch (e) {
